...
- **使用中账号识别**：CPA 的逐请求用量队列喂给本地 7 天账本，悬浮卡片显示最近 24 小时真正服务过当前模型的账号（按最后使用排序，带「使用中」徽章与 24h 请求/Token 统计），其余折叠进「其它账号」。
...
- **悬浮卡片账号变少了？** 「只显示使用中」开关默认开启：最近 24 小时实际服务过当前模型的账号会置顶并带「使用中」徽章，其余折叠在「其它账号」里。关掉卡片上的开关即可回到全量候选视图。
