...
`;
			...
			return h("div", { className: "cpa-q-card" },
				...
				open ? h("div", { className: "cpa-q-card-body" },
					...
					h("div", { className: "cpa-q-inst-row cpa-q-toggle-row" },
						...
						ZH ? "只显示使用中的账号（每个刷新周期内用过当前模型）" : "Only show accounts that served the current model in the current refresh cycle"),
					...rows.map((row) => {
		...
		function apply(ctx, configArg) {
			...
			// "In use" tracks the current refresh cycle (also set on config changes).
			usageWindowMs = refreshMs;
			...
			function applyConfig(next) {
				...
				// "In use" tracks the current refresh cycle.
				usageWindowMs = refreshMs;
				rebuildInstances();
				scheduleDecorate();
				restartQuotaTimer();
