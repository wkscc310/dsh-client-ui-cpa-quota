	...
	factory: (require) => {
		...
		async function probeCpa(base) {
			try {
				...
				if (response.status >= 200 && response.status < 300 && /management|logging|enabled|true|false/i.test(body)) return "cpa";
				return "unknown";
			} catch {
				return "unreachable";
			}
		...
		async function apiCall(instance, payload, timeoutMs = 0) {
		...
		async function queryAccount(instance, entry) {
			...
			try {
				if (provider === "codex") {
					...
					let lastError = "";
					for (const url of [...UPSTREAM.antigravityQuota, ...UPSTREAM.antigravityModels]) {
						try {
		...
		async function refreshInstance(instance) {
			...
			const accounts = await Promise.all(entries
				...
				.map((entry) => queryAccount(instance, entry)));
...
`;
		...
		function QuotaSettingsCard() {
			...
			const commit = (rows) => {
				saveLocalInstances(rows.map((r) => ({ baseURL: r.baseURL, key: keyOf(r) })), undefined);
			};
			...
			return h("div", { className: "cpa-q-card" },
				h("div", { className: "cpa-q-card-head" },
					h("div", null,
					...
					h("div", { className: "cpa-q-refresh-control" },
		...
		function apply(ctx, configArg) {
			...
			async function refreshModelIndex() {
				...
				if (disposed) return;
				rebuildInstances();
				await probeProviderBases(providerBases, false);
				if (disposed) return;
				rebuildInstances();
				scheduleDecorate();
			}
			...
			async function refreshQuota() {
				...
				try {
					...
					await Promise.all(targets.map(async (instance) => {
						const key = baseKey(instance.baseURL);
						// Keyless auto-detected instances never hit the management API.
						if (instance.managementKey === "" && !config.instances.some((i) => baseKey(i.baseURL) === key)) {
			...
			const onRefreshRequested = () => void refreshQuota();
			...
			// Settings card: Settings → Plugins → “CliProxyAPI Quota”.
			if (ctx.slots !== undefined && typeof ctx.slots.inject === "function") {
				ctx.effect(() => ctx.slots.inject("settings.plugin.item", () => ctx.slots.register(
					{ name: "settings.plugin.item", id: "cpa-quota", order: 6 },
					QuotaSettingsCard,
				)), "ui-cpa-quota: settings card");
			}
