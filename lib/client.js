...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function parseImportedConfig(raw) {
			...
			// Mirror readConfig's clamp so an exported config round-trips its
			// usage window instead of silently resetting it to the default.
			if (parsed.usageWindowMinutes !== undefined) {
				const window = numberOrNull(parsed.usageWindowMinutes);
				if (window !== null && window >= 5) out.usageWindowMinutes = Math.min(10080, window);
			}
		...
		function enableUsageStats(instance) {
			// mgmtBase keeps the scheme; baseKey is the display/match key and
			// strips it, which produced a scheme-less URL ("host/v0/...") that
			// fetch() rejects with "unknown scheme".
			const key = baseKey(instance.baseURL);
			return fetchJSON(`${mgmtBase(instance.baseURL)}/v0/management/usage-statistics-enabled`, {
				method: "PUT",
				headers: {
				...
				usageStatsMemory.delete(key);
				shared.usageStatsByBase[key] = true;
			}).catch(() => {
				// Drop the cached verdict so the next refresh re-probes, and the
				// caller does not get an unhandled rejection.
				usageStatsMemory.delete(key);
			});
		}
...
`;
								...
								picker.onchange = () => {
									...
									reader.onload = () => {
										try {
											...
											saveLocalInstances(imported.instances.map((i) => ({ baseURL: i.baseURL, key: i.managementKey })), imported.refreshMinutes, imported.usageWindowMinutes);
										} catch (error) {
		...
		function apply(ctx, configArg) {
			...
			let tipHideTimer = 0;
			let tipAnchor = null;
			/** "Other accounts" group expanded; survives re-renders for the same anchor. */
			let tipOthersOpen = false;
			...
			/**
			 * Place the card next to its ring. Preferred side is above; the card
			 * is then anchored by its bottom edge so later growth (expanding the
			 * "other accounts" group, a refresh adding rows) extends upward into
			 * free space instead of sliding under the ring and off the viewport.
			 * Whichever side wins, max-height is the room actually available
			 * there, so content beyond it scrolls inside the card.
			 */
			function placeTooltip(dot) {
				...
				const margin = 8;
				const gap = 8;
				const cap = Math.min(560, Math.floor(window.innerHeight * 0.7));
				const spaceAbove = Math.floor(rect.top - gap - margin);
				const spaceBelow = Math.floor(window.innerHeight - rect.bottom - gap - margin);
				const need = (tip.scrollHeight || 0) + 2;
				const above = need <= spaceAbove || spaceAbove >= spaceBelow;
				const room = Math.max(120, Math.min(cap, above ? spaceAbove : spaceBelow));
				tip.style.maxHeight = `${room}px`;
				// Measure width at the left edge: a fixed box near the right edge
				// would otherwise shrink to the space left of the viewport border.
				tip.style.left = "0px";
				...
				if (above) {
					tip.style.top = "auto";
					tip.style.bottom = `${Math.round(window.innerHeight - rect.top + gap)}px`;
				} else {
			...
			function showTooltip(dot) {
				...
				// A re-render for the same ring (quota refresh, model relabel) must
				// not throw away what the user did with the card.
				const sameAnchor = tip !== null && tipAnchor === dot;
				const keepScroll = sameAnchor ? tip.scrollTop || 0 : 0;
				if (!sameAnchor) tipOthersOpen = false;
				...
				if (state === undefined) {
					...
					if (split.others.length > 0 && split.active.length > 0) {
						...
						const othersLabel = ZH ? `其它账号 (${othersCount})` : `Other accounts (${othersCount})`;
						const othersHead = el("div", "cpa-q-others-head", (tipOthersOpen ? "▾ " : "▸ ") + othersLabel);
						const othersBody = el("div", "cpa-q-others-body");
						othersBody.style.display = tipOthersOpen ? "block" : "none";
						...
						othersHead.addEventListener("click", () => {
							tipOthersOpen = othersBody.style.display === "none";
							othersBody.style.display = tipOthersOpen ? "block" : "none";
							othersHead.textContent = (tipOthersOpen ? "▾ " : "▸ ") + othersLabel;
							// The card just changed height: re-fit it to the room on its side.
							if (tipAnchor === dot) placeTooltip(dot);
						});
				...
				placeTooltip(dot);
				if (keepScroll > 0) tip.scrollTop = keepScroll;
			}
			...
			// The model trigger's label span is not the first child on every
			// host: dsh 0.1.5+ renders an svg icon before it. The button's
			// title carries the raw "model · effort" label on every version
			// so far, so prefer it and fall back to the first text-bearing
			// direct-child span for hosts without one.
			function triggerLabelOf(button) {
			...
			function decorate() {
				...
				for (const button of document.querySelectorAll('button[aria-haspopup="menu"]')) {
					const label = triggerLabelOf(button);
					if (label === "") continue;
			...
			const onScrollOrResize = (event) => {
				if (tip === null || tipAnchor === null) return;
				// The listener captures every scroll in the document, including the
				// card's own internal scrolling; re-placing on those would reset the
				// user's position. Page scroll and resize only move the anchor.
				if (event !== undefined && event.type === "scroll" && event.target !== undefined && event.target !== null
					&& typeof tip.contains === "function" && (event.target === tip || tip.contains(event.target))) return;
				placeTooltip(tipAnchor);
			};
