...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function readConfig(...sources) {
			const merged = { instances: [], refreshMinutes: 10 };
		...
		async function loadModelIndex(api) {
			const [providersRes, settingsRes] = await Promise.all([api.llm.providers({}), api.settings.describe({})]);
			if (!providersRes.result.ok) throw new Error(`llm.providers: ${providersRes.result.error?.message ?? "failed"}`);
			if (!settingsRes.result.ok) throw new Error(`settings.describe: ${settingsRes.result.error?.message ?? "failed"}`);
			const namespaces = new Map(settingsRes.result.value.namespaces.map((view) => [view.ns, view]));
			const index = new Map();
			const providerBases = new Map();
			for (const entry of providersRes.result.value.providers) {
				const view = namespaces.get(entry.settingsNs);
				if (view === undefined) continue;
				const configValue = entry.settingsPath.length === 0 ? view.value : getPath(view.value, entry.settingsPath);
				if (configValue === null || typeof configValue !== "object") continue;
				const baseURL = text(configValue.baseURL);
				...
				for (const model of models) {
					...
					if (id === "") continue;
					const record = { baseURL, modelId: id, providerId: text(entry.provider) };
					index.set(id.toLowerCase(), record);
		 ...
		 * same series (effort suffixes stripped), or a pure extension either way. */
		...
		async function queryAccount(instance, entry) {
			...
			const account = {
				name: firstText(entry.label, entry.email, entry.account_email, entry.name, entry.id, authIndex, "unknown"),
				provider,
				authIndex,
				modelList: null,
		...
		/**
		 * Accounts whose served models match the selected one, in three tiers:
		 * 1. accounts that DECLARE their model list (CPA's per-auth routing
		 *    registry) and list this model — authoritative, wins outright, and
		 *    a declared list without the model excludes that account from every
		 *    fallback below;
		 * 2. family heuristics over the remaining accounts — the model name
		 *    first, then the DSH provider id that serves it;
		 * 3. when no family can be determined, CPA's routing decision is
		 *    unknowable from the client — every usable account is listed and
		 *    the tooltip says why.
		 */
		function accountsForModel(accounts, modelId, providerHint = "") {
			if (!Array.isArray(accounts)) return [];
			if (text(modelId) === "") return accounts;
			...
			const exact = listed.filter((account) => account.modelList.some((id) => modelListMatches(id, modelId)));
			...
			// Cross-channel models (Claude served by both Antigravity and Claude
			// OAuth, for example) are load-balanced by CPA between the channels,
			// so specific-window accounts and generic family matches union — the
			// provider badge tells the user which channel each account is on.
			const specific = usable(unlisted).filter((account) => modelSpecificWindows(account, modelId).length > 0);
			const generic = usable(unlisted).filter((account) => {
			...
			if (specific.length > 0 || generic.length > 0) return [...specific, ...generic];
			// Family known but nothing serves it: the honest answer is "no
			// matching account", not a list of guesses.
			return [];
		}
		...
		function relevantWindows(account, modelId) {
			...
			function levelFor(state, modelId, providerHint = "") {
				...
				const matchingAccounts = accountsForModel(state.accounts, modelId, providerHint);
			const rows = [];
			for (const account of matchingAccounts) {
				if (account.disabled || account.unavailable) continue;
				for (const w of modelRowsForAccount(account, modelId)) rows.push(w);
			}
			if (rows.length === 0) return { level: matchingAccounts.length > 0 ? "ok" : "pending", remaining: null };
...
.cpa-q-card-dot{flex:none;width:9px;height:9px;border-radius:999px;margin-top:5px;background:var(--dsw-alias-label-disabled,#9ca3af)}
.cpa-q-card-dot[data-level="ok"]{background:var(--dsw-alias-state-success-primary,#22c55e)}
.cpa-q-card-dot[data-level="low"]{background:#f59e0b}
.cpa-q-card-dot[data-level="exhausted"],.cpa-q-card-dot[data-level="error"]{background:#ef4444}
...
`;
		...
		/**
		 * Worst window across every known instance, for the collapsed card's
		 * health dot: exhausted/error → red, any window under 20% → amber, data
		 * with headroom → green, nothing readable yet → gray.
		 */
		function overallHealth() {
			const rank = { pending: 0, ok: 1, low: 2, exhausted: 3, error: 3 };
			let worst = { level: "pending", remaining: null };
			for (const instance of shared.instances) {
				const state = quotaByKey.get(baseKey(instance.baseURL));
				if (state === undefined) continue;
				let agg;
				if (state.state === "error") agg = { level: "error", remaining: null };
				else if (state.state !== "ok" || !Array.isArray(state.accounts) || state.accounts.length === 0) agg = { level: "pending", remaining: null };
				else agg = levelFor(state, "");
				if (rank[agg.level] > rank[worst.level]) worst = agg;
			}
			return worst;
		}

		function healthTitle(level) {
			if (level === "ok") return ZH ? "额度健康：所有窗口剩余 ≥20%" : "healthy: every window has ≥20% left";
			if (level === "low") return ZH ? "额度紧张：有窗口剩余不足 20%" : "low: a window is under 20%";
			if (level === "exhausted") return ZH ? "有窗口已耗尽" : "a window is exhausted";
			if (level === "error") return ZH ? "有实例刷新失败" : "an instance failed to refresh";
			return ZH ? "等待首次刷新，或尚未配置密钥" : "waiting for first refresh, or the key is not set";
		}
		 ...
		 */
		function accountSection(account, modelId) {
		...
		function saveLocalInstances(rows, refreshMinutes) {
			writeLocalConfig({
				instances: rows
		...
		function QuotaSettingsCard() {
			...
			// quota panel. The dot condenses every instance's worst window so the
			// collapsed card alone answers "is anything out of quota?".
			...
			return h("div", { className: "cpa-q-card" },
				h("button", {
					...
					h("div", { className: "cpa-q-card-heading" },
						h("span", { className: "cpa-q-card-dot", "data-level": health.level, title: healthTitle(health.level) }),
						h("div", null,
		...
		//#region plugin body
		// 'connection' carries the RPC face (provider directory + settings);
		// 'slots' is required before ctx.slots is readable on the proxy context.
		...
		function apply(ctx, configArg) {
			...
			async function refreshModelIndex(forceProbe = false) {
				try {
					...
					const loaded = await loadModelIndex(api);
				...
				if (state === undefined) {
					...
					const matchingAccounts = accountsForModel(state.accounts, modelId, providerHint);
						if (matchingAccounts.length === 0) {
							...
							if (modelFamily(modelId) === "" && providerFamilyFromId(providerHint) === "") {
							...
							for (const account of matchingAccounts) tip.appendChild(accountSection(account, modelId));
						}
			...
			function applyDotState(ring) {
				...
				const agg = levelFor(quotaByKey.get(key), modelId, ring.getAttribute("data-cpa-provider") ?? "");
