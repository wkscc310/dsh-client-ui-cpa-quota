...
export function normalizeConfig(value) {
	...
	const usageWindowMinutes = Number(source.usageWindowMinutes);
	return {
		refreshMinutes: Number.isFinite(refreshMinutes) && refreshMinutes >= 1 ? Math.min(1440, refreshMinutes) : 10,
		// The browser half clamps the in-use detection window to [5, 10080];
		// keep the same range here so a yaml-provided window survives the
		// host-side normalization the browser config is piped through.
		usageWindowMinutes: Number.isFinite(usageWindowMinutes) && usageWindowMinutes >= 5 ? Math.min(10080, usageWindowMinutes) : 1440,
...
export const Config = Object.assign(
	(value) => normalizeConfig(value),
	{
		...
		dict: {
			...
			usageWindowMinutes: { type: "number" },
