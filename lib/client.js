...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function codexAccountId(entry) {
			const candidates = [entry.id_token, getPath(entry, ["metadata", "id_token"]), getPath(entry, ["attributes", "id_token"])];
		...
		async function probeCpa(base) {
			try {
				const response = await fetch(`${base}/v0/management/usage-statistics-enabled`, { method: "GET" });
				const body = await response.text();
		...
		async function probeProviderBases(providerBases, force) {
			...
			for (const [key, info] of providerBases) {
				const hit = cache[key];
				if (!force && hit !== undefined && Date.now() - hit.at < PROBE_TTL) {
		...
		async function queryAccount(instance, entry) {
			...
			try {
				if (provider === "codex") {
					...
					if (accountID === "") {
						account.error = ZH ? "缺少 chatgpt_account_id" : "missing chatgpt_account_id";
						return account;
					}
		...
		/** Accounts whose provider/model rows match the selected model. */
		function accountsForModel(accounts, modelId) {
			if (!Array.isArray(accounts)) return [];
			const family = modelFamily(modelId);
			if (text(modelId) === "" || family === "") return accounts;
		...
		function levelFor(state, modelId) {
			...
			const matchingAccounts = accountsForModel(state.accounts, modelId);
			const rows = [];
...
`;
		...
		function apply(ctx, configArg) {
			...
			function applyDotState(ring) {
				...
				const agg = levelFor(quotaByKey.get(key), modelId);
