...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * The ChatGPT account id for a Codex auth file. CPA stores it in more
		 * than one place depending on version: `tokens.account_id` is the
		 * canonical field in current builds, older ones only carry the JWT
		 * `id_token`, and metadata/attributes copies appear on the management
		 * API projection.
		 */
		...
		/**
		 * Accounts whose provider/model rows match the selected model. Two
		 * signals: the model name's family, then the DSH provider id that serves
		 * it (custom route names reveal nothing on their own). When neither
		 * yields a family, CPA's routing decision is unknowable from the client,
		 * so every usable account is listed and the tooltip says why.
		 */
		function accountsForModel(accounts, modelId, providerHint = "") {
			...
			if (family === "") return accounts.filter((account) => !account.disabled && !account.unavailable);
			const specific = accounts.filter((account) => modelSpecificWindows(account, modelId).length > 0);
